<div class="row row-form">
    <div id="breadcrumbBasic" class="col-xl-12 col-lg-12 layout-spacing">
        <div class="statbox widget box box-shadow">
                <div class="widget-content widget-content-area">

                <nav class="breadcrumb-one float-left">
                    <a href="staffprofile">Back to Profile</a>
                </nav>

                <nav class="breadcrumb-one float-right" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="logout">Logout</a></li>
                        <!-- <li class="breadcrumb-item active" aria-current="page"><span>UI Kit</span></li> -->
                    </ol>
                </nav>
            </div>
        </div>
    </div>
</div>

<div class="row invoice layout-top-spacing">
    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
        <div class="row inv--head-section justify-content-md-center">
            <!-- <div class="col-lg-8 col-offset-2"> -->
                <!-- <div class="col-12 layout-spacing justify-content-md-center">
                    <h3 class="in-heading">ASSESSMENT SETUP</h3>
                </div> -->
                <!-- <div class="col-lg-6">
                    <h3 class="in-heading">ASSESSMENT SETUP</h3>
                </div> -->
                <div class="col-lg-6">
                    <h3 class="in-heading">BILL SETUP</h3>
                    <br>
                    <!-- <div class="form-group">
                        <label for="t-text">Select Class: </label> &ast;
                        <div class="form-group">
                            <select class="selectpicker form-control" id="classgroup" name="classgroup" required>
                                <option disabled selected>--Choose One--</option>
                                <option value="A">Primary 4b</option>
                                <option value="B">Primary 2a</option>
                                <option value="C">Primary 5c</option>
                                <option value="D">Primary 1a</option>
                            </select>
                        </div>
                    </div> -->
                </div>
            <!-- </div> -->
        </div>
        <div class="app-hamburger-container">
            <div class="hamburger"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-menu chat-menu d-xl-none"><line x1="3" y1="12" x2="21" y2="12"></line><line x1="3" y1="6" x2="21" y2="6"></line><line x1="3" y1="18" x2="21" y2="18"></line></svg></div>
        </div>
        <div class="doc-container">
            <div class="tab-title">

                <div class="row">
                    <div class="col-md-12 col-sm-12 col-12">
                        <!-- <div class="search">
                            <input type="text" class="form-control" placeholder="Search">
                        </div>
                        <div class="col-md-12 col-sm-12 col-12">
                            <h4 class="title-heading"><strong>List of Classes</strong></h4>
                        </div> -->
                        <div class="widget-content widget-content-area br-6">
                            <h4 class="title-heading"><strong><u>LIST OF CLASSES</u></strong></h4>
                            <div class="table-responsive mb-4 mt-4">
                                <table id="studentprofiletable" class="table table-hover" style="width:100%">
                                    <thead>
                                        <tr>
                                            <th>S/N.</th>
                                            <th>Name</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr class="list-actions" id="invoice-00001" data-invoice-id="00001">
                                            <td>1</td>
                                            <td>Junior Secondary School 1</td>
                                        </tr>
                                        <tr class="list-actions" id="invoice-00002" data-invoice-id="00002">
                                            <td>2</td>
                                            <td>Junior Secondary School 2</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                        <!-- <ol class="nav nav-pills inv-list-container d-block" id="pills-tab" role="tablist">
                            <li class="nav-item">
                                <div class="nav-link list-actions" id="invoice-00001" data-invoice-id="00001">
                                    <div class="f-m-body">
                                        <div class="f-head">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-dollar-sign"><line x1="12" y1="1" x2="12" y2="23"></line><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"></path></svg>
                                        </div>
                                        <div class="f-body">
                                            <p class="invoice-number">Student ID #00001</p>
                                            <p class="invoice-customer-name"> Jesse Cory</p>
                                            <p class="invoice-generated-date">Date: 12 Apr 2019</p>
                                        </div>
                                    </div>
                                </div>
                            </li>
                        </ol> -->
                    </div>
                </div>
            </div>

            <div class="invoice-container">
                <div class="invoice-inbox">

                    <div class="inv-not-selected">
                        <p>Select a class from above to setup the Bills.</p>
                    </div>

                    <!-- <div class="invoice-header-section">
                        <h4 class="inv-number"></h4>
                        <div class="invoice-action">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-printer action-print" data-toggle="tooltip" data-placement="top" data-original-title="Reply"><polyline points="6 9 6 2 18 2 18 9"></polyline><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"></path><rect x="6" y="14" width="12" height="8"></rect></svg>
                        </div>
                    </div> -->
                    
                    <div id="ct" class="">
                        
                        <div class="invoice-00001">
                            <div class="content-section  animated animatedFadeInUp fadeInUp">

                                <div class="row inv--product-table-section">
                                    <div class="col-lg-5">
                                        <div class="row inv--head-section">
                                            <!-- <div class="col-12 layout-spacing">
                                                <h3 class="in-heading">Observable Traits' Setup</h3>
                                            </div>
                                            <br> -->
                                            <div class="col-12 layout-spacing">
                                                <h4><span>Class ID:</span> <?php echo "classid"?></h4>
                                                <h4><span>Class Names:</span> <?php echo "classnames"?></h4>
                                            </div>
                                        </div>
                                        <div class="table table-hover">
                                            <table id="assessment1" style="width:100%">
                                                <thead>
                                                    <tr>
                                                        <th width="40%" scope="col">Bill Type</th>
                                                        <th width="60%" scope="col">Amount</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr>
                                                        <td>School Fees</td>
                                                        <td class="text-right"><input type="text" id="schoolfees" name="schoolfees" class="form-control"></td>
                                                    </tr>
                                                    <tr>
                                                        <td>Computer</td>
                                                        <td class="text-right"><input type="text" id="computerfees" name="computerfees" class="form-control"></td>
                                                    </tr>
                                                    <tr>
                                                        <td>Lesson</td>
                                                        <td class="text-right"><input type="text" id="lessonfees" name="lessonfees" class="form-control"></td>
                                                    </tr>
                                                    <tr>
                                                        <td>P.T.A</td>
                                                        <td class="text-right"><input type="text" id="ptafees" name="ptafees" class="form-control"></td>
                                                    </tr>
                                                    <tr>
                                                        <td>Head Teacher's Name</td>
                                                        <td class="text-right"><input type="text" id="headteachersname" name="headteachersname" class="form-control"></td>
                                                    </tr>
                                                    <tr>
                                                        <td>Date of Vacation</td>
                                                        <td class="text-right">
                                                            <input id="basicFlatpickr4" name="vacationdate" class="form-control flatpickr flatpickr-input active" type="text" placeholder="Select Date.." required>
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td>Date of Resumption</td>
                                                        <td class="text-right">
                                                            <input id="basicFlatpickr1" name="resumptiondate" class="form-control flatpickr flatpickr-input active" type="text" placeholder="Select Date.." required>
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td>Next Term Begins</td>
                                                        <td class="text-right">
                                                            <input id="basicFlatpickr2" name="nexttermbegins" class="form-control flatpickr flatpickr-input active" type="text" placeholder="Select Date.." required>
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td>Next Term Ends</td>
                                                        <td class="text-right">
                                                            <input id="basicFlatpickr3" name="nexttermends" class="form-control flatpickr flatpickr-input active" type="text" placeholder="Select Date.." required>
                                                        </td>
                                                    </tr>
                                                </tbody>
                                                <tfoot>
                                                    <tr>
                                                        <th width="40%" scope="col">Bill Type</th>
                                                        <th width="60%" scope="col">Amount</th>
                                                    </tr>
                                                </tfoot>
                                            </table>
                                        </div>
                                        <div class="col text-right">
                                            <button class="btn btn-primary mb-4 mr-2 btn-lg">Save Bills</button>
                                        </div>

                                    </div>
                                    <div class="col-lg-7 border-left">
                                        <div class="main-container sidebar-closed sbar-open" id="container">
                                            <!--  BEGIN CONTENT AREA  -->        
                                        
                                            <div class="col-xl-12 col-lg-12 col-sm-12 ">

                                            <h3 style="margin-bottom: 20px;">Bill Setup List</h3>

                                            <div class="widget-content widget-content-area br-6">
                                                <div class="table-responsive mb-4 mt-4">
                                                <table id="studentprofiletable" class="table table-hover" style="width:100%">
                                                    <thead>
                                                    <tr>
                                                        <th>Class ID.</th>
                                                        <th>Class Name</th>
                                                        <th>Remarks</th>
                                                        <th class="no-content">Action</th>
                                                    </tr>
                                                    </thead>
                                                </table>
                                                </div>
                                            </div>
                                            </div>    
                                            <!--  END CONTENT AREA  -->
                                        </div>
                                        
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="invoice-00002">
                            <div class="content-section  animated animatedFadeInUp fadeInUp">

                                <div class="row inv--product-table-section">
                                    <div class="col-lg-5">
                                        <div class="row inv--head-section">
                                            <!-- <div class="col-12 layout-spacing">
                                                <h3 class="in-heading">Observable Traits' Setup</h3>
                                            </div>
                                            <br> -->
                                            <div class="col-12 layout-spacing">
                                                <h4><span>Class ID:</span> <?php echo "classid"?></h4>
                                                <h4><span>Class Names:</span> <?php echo "classnames"?></h4>
                                            </div>
                                        </div>
                                        <div class="table table-hover">
                                            <table id="assessment1" style="width:100%">
                                                <thead>
                                                    <tr>
                                                        <th width="40%" scope="col">Bill Type</th>
                                                        <th width="60%" scope="col">Amount</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr>
                                                        <td>School Fees</td>
                                                        <td class="text-right"><input type="text" id="row-1-ass1" name="row-1-ass1" class="form-control"></td>
                                                    </tr>
                                                    <tr>
                                                        <td>Computer</td>
                                                        <td class="text-right"><input type="text" id="row-1-ass1" name="row-1-ass1" class="form-control"></td>
                                                    </tr>
                                                    <tr>
                                                        <td>Lesson</td>
                                                        <td class="text-right"><input type="text" id="row-1-ass1" name="row-1-ass1" class="form-control"></td>
                                                    </tr>
                                                    <tr>
                                                        <td>P.T.A</td>
                                                        <td class="text-right"><input type="text" id="row-1-ass1" name="row-1-ass1" class="form-control"></td>
                                                    </tr>
                                                    <tr>
                                                        <td>Head Teacher's Name</td>
                                                        <td class="text-right"><input type="text" id="row-1-ass1" name="row-1-ass1" class="form-control"></td>
                                                    </tr>
                                                    <tr>
                                                        <td>Date of Vacation</td>
                                                        <td class="text-right">
                                                            <input id="basicFlatpickr" name="dob" class="form-control flatpickr flatpickr-input active" type="text" placeholder="Select Date.." required>
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td>Date of Resumption</td>
                                                        <td class="text-right">
                                                            <input id="basicFlatpickr1" name="dateofaward" class="form-control flatpickr flatpickr-input active" type="text" placeholder="Select Date.." required>
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td>Next Term Begins</td>
                                                        <td class="text-right">
                                                            <input id="basicFlatpickr2" name="dateofaward" class="form-control flatpickr flatpickr-input active" type="text" placeholder="Select Date.." required>
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td>Next Term Ends</td>
                                                        <td class="text-right">
                                                            <input id="basicFlatpickr3" name="dateofaward" class="form-control flatpickr flatpickr-input active" type="text" placeholder="Select Date.." required>
                                                        </td>
                                                    </tr>
                                                </tbody>
                                                <tfoot>
                                                    <tr>
                                                        <th width="40%" scope="col">Bill Type</th>
                                                        <th width="60%" scope="col">Amount</th>
                                                    </tr>
                                                </tfoot>
                                            </table>
                                        </div>
                                        <div class="col text-right">
                                            <button class="btn btn-primary mb-4 mr-2 btn-lg">Save Bills</button>
                                        </div>

                                    </div>
                                    <div class="col-lg-7 border-left">
                                        <div class="main-container sidebar-closed sbar-open" id="container">
                                            <!--  BEGIN CONTENT AREA  -->        
                                        
                                            <div class="col-xl-12 col-lg-12 col-sm-12 ">

                                            <h3 style="margin-bottom: 20px;">Bill Setup List</h3>

                                            <div class="widget-content widget-content-area br-6">
                                                <div class="table-responsive mb-4 mt-4">
                                                <table id="studentprofiletable" class="table table-hover" style="width:100%">
                                                    <thead>
                                                    <tr>
                                                        <th>Class ID.</th>
                                                        <th>Class Name</th>
                                                        <th>Remarks</th>
                                                        <th class="no-content">Action</th>
                                                    </tr>
                                                    </thead>
                                                </table>
                                                </div>
                                            </div>
                                            </div>    
                                            <!--  END CONTENT AREA  -->
                                        </div>
                                        
                                    </div>
                                </div>
                            </div>
                        </div> 
                    </div>


                </div>

                <!-- <div class="inv--thankYou">
                    <div class="row">
                        <div class="col-sm-12 col-12">
                            <p class="">Thank you for doing Business with us.</p>
                        </div>
                    </div>
                </div> -->

            </div>
            
        </div>

    </div>
    </div>


<script type="text/javascript">
    $('a').on('click', function (e) {
        e.preventDefault();
        var $href = $(this).attr('href');
        $(this).addClass('active');

        $('#' + $href).addClass('active'); // this is what you need to do
    });
</script>